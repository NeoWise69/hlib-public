	...
	: m_size(other.m_size)
{
	...
	for (pointer it = *other.m_buckets.first; it; it = it->next) {
		unordered_hash_node<Key, void>* newnode = new(placeholder(), Alloc::allocate(sizeof(unordered_hash_node<Key, void>))) unordered_hash_node<Key, void>(*it);
...
inline void unordered_set<Key, Alloc>::clear() {
	...
	while (it) {
		...
		Alloc::deallocate(it, sizeof(unordered_hash_node<Key, void>));
...
inline pair<typename unordered_set<Key, Alloc>::iterator, bool> unordered_set<Key, Alloc>::insert(const Key& key) {
	...
	unordered_hash_node<Key, void>* newnode = new(placeholder(), Alloc::allocate(sizeof(unordered_hash_node<Key, void>))) unordered_hash_node<Key, void>(key);
	newnode->next = newnode->prev = 0;
...
inline pair<typename unordered_set<Key, Alloc>::iterator, bool> unordered_set<Key, Alloc>::emplace(Key&& key) {
	...
	unordered_hash_node<Key, void>* newnode = new(placeholder(), Alloc::allocate(sizeof(unordered_hash_node<Key, void>))) unordered_hash_node<Key, void>(static_cast<Key&&>(key));
	newnode->next = newnode->prev = 0;
...
inline void unordered_set<Key, Alloc>::erase(iterator where) {
	...
	Alloc::deallocate((void*)where.node, sizeof(unordered_hash_node<Key, void>));
	--m_size;
}
